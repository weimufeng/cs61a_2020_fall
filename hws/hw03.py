...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        ...
        if label(t) == find_value:
            return tree(find_value, [replace_leaf(b, find_value, replace_value) for b in branches(t)])
        else:
...
def has_path(t, word):
    ...
    if len(word) == 1:
        ...
        return word[0] == label(t) and any([has_path(b, word[1:]) for b in branches(t)])
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) < 0 or 0 < lower_bound(y) 
