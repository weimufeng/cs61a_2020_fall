...
def time_per_word(times_per_player, words):
    ...
    for item in times_per_player:
        ls += [[item[i]-item[i-1] for i in range(1, len(item))]]
...
def fastest_words(game):
    ...
    for j in word_indices:
        minimun = time(game, 0, j)
        for i in player_indices:
            if time(game, i, j) <= minimun:
                minimun = time(game, i, j)
        fastest_time_ls.append(minimun)
    
    player_for_min_ls = []
    for j in word_indices:
        player_for_word = []
        for i in player_indices:
            if time(game, i, j) == fastest_time_ls[j]:
                player_for_word.append(i)
        player_for_min_ls.append(player_for_word)
    
    player_match_word_ls = []
    for k in player_for_min_ls:
        s = min(k)
        player_match_word_ls.append(s)
    
    results = []
    for i in player_indices:
        result_for_player = []
        for j in word_indices:
            if i == player_match_word_ls[j]:
                result_for_player.append(word_at(game, j))
        results.append(result_for_player)
